...
//
// Position-based dynamics: predict where every fruit wants to be, then push
// overlaps apart a few times per step and read the velocity back off the
// movement. Piles of thirty fruits settle without the jitter an
...
export class World {
      ...
      for (let i = 0; i < bodies.length; i++) {
        ...
        for (let j = i + 1; j < bodies.length; j++) {
          ...
          if (first) {
            ...
            // Friction, purely so fruit spins as it rolls off a shoulder.
  ...
  overlappingPairs(sameTierOnly = true) {
    const out = [];
    for (const [a, b] of this.contacts) {
      if (a.dead || b.dead) continue;
      if (sameTierOnly && a.tier !== b.tier) continue;
