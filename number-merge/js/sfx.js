...
export class Sfx {
  ...
  // Each rung up the ladder pops a note higher, so the doubling is audible.
  merge(tier)   { const f = 320 * Math.pow(1.09, tier); this.tone(f, 0.16, 'sine', f * 1.6, 0.9); }

  // A Zero vanishing into something: a small sound that goes nowhere, because
  // nothing was made.
  absorb()      { this.tone(300, 0.14, 'sine', 300, 0.28); }
  ...
  newBiggest()  { [0, 0.09, 0.18].forEach((d, i) => setTimeout(() => this.tone(520 * Math.pow(1.26, i), 0.16, 'sine', null, 0.5), d * 1000)); }
