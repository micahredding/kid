...
### 2026-08-22 (Number Merge wouldn't load on the iPad — boot hardening)

Reported: can't get the game to load on the iPad.

- **Not a JS-support problem.** Checked the served files for anything needing a late Safari (`.at()`, `replaceAll`, `Object.hasOwn`, `structuredClone`, `Promise.any`, logical assignment, numeric separators, private fields): clean. Top-level `await` is used, but `fruit-merge` uses the identical pattern and works on that iPad. And mini4's logs show number-merge played through on **Mac Safari** — same WebKit — including a 448-drop game to 512 for 6890. The iPad has logged **no `start` event at all**, so it either never reached the page or died before the module finished.
- Two real defects of mine were in that path, both able to present exactly as "won't load", and both fixed regardless of which one bit:
  - **`await fetch('./art/blocks.json').then(r => r.json())` had no catch.** `sw.js` answers a cache miss it cannot fetch with a *plain-text* 503 body, so `.json()` throws, the module dies mid-evaluation, and `LOADING ASHER'S NUMBERS…` stays on screen for ever. Now: retry once, then fall back.
  - **`new Image()` with only `onload`/`onerror` never settles if a request stalls** rather than failing. One hung sprite held the whole `Promise.all` and the game never started. Every sprite now has a 6s deadline.
- **`fallbackBlocks()` in `config.js` builds a complete ladder from nothing but that file** — no manifest, no sprites. The ladder is arithmetic and the numeral is painted on the ball, so plain coloured discs are fully playable; rung colour comes from the first digit, which is the paint he used most on 11 of 13 (Sixteen and 2048 are where a later digit dominated). Verified by rendering the game against a server that 503s all of `/art/`: it boots, titles, charts all thirteen rungs and plays.
- **A boot watchdog in a classic `<script>`**, because nothing inside a module can report a module that fails to *parse*. After 9s it replaces the LOADING line with `COULD NOT START`, the path, and tap-to-reload. Verified against a server that 503s `/js/`. Also an `unhandledrejection` handler that puts the error on screen — the iPad has no console.
- Same hardening ported to `fruit-merge` (identical hang paths). It gets the watchdog, the retry and the sprite deadlines, but **no synthesised fallback** — the fruit *are* only their drawings, so a failed manifest says so and offers a reload rather than inventing eleven anonymous discs.
- **`WORDS` moved into `js/config.js`** as the single source, with `tools/extract_art.mjs` importing it instead of keeping a second copy. The fallback needs the words, and two copies would drift. A test asserts the manifest's words still equal config's.
- **Hub: a newly added game needed a second manual reload before it could be launched at all.** `sw.js` serves the hub cache-first, so an already-open page keeps running the JS it loaded — including its hardcoded `GAMES` list — and the terminal simply did not know the name. `kid/index.html` now reloads once on `controllerchange` (by which point the cache is updated), guarded: never over an open game overlay, never mid-typing, never more than once. This is the most likely explanation for the actual report, and it is a class fix for every future game.
- number-merge 127 checks, fruit-merge 76, all green.
